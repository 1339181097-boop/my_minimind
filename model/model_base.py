...
class CaveManMindConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        num_attention_heads: int = 8, # q分头数
        num_hidden_layers: int = 8,   # transformer层数
        num_key_value_heads: int = 2, # k,v分头数
        vocab_size: int = 6400,       
...
# 将1/n_rep的kv重复n_rep次
#在显存敏感的大模型推理中，repeat_interleave 会物理复制内存
#导致显存占用激增。而 expand 是创建视图 (View)，不占用额外显存
def repeat_kv(x,n_rep):
    if n_rep==1:
        return x
    else:   # 可以用expend和reshape优化计算速度
        x=torch.repeat_interleave(x,n_rep,dim=2)# 在头的位置上进行复制
        return x
    ...
    def forward(self,X,
        ...
        # 因果掩码（需要自己生成）
            ...
            if attention_mask is not None:
                # 确保 mask 维度与 scores 匹配 (Batch, 1, 1, Key_Len) 或广播兼容
                # 假设 attention_mask 为 1 代表保留，0 代表掩盖
