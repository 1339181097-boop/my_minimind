...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    # 所有1的数量就是有效token数量，并且即使长度为0也强制给一个极小数，防止除0
    seq_lengths = mask.sum(dim=1, keepdim=True).clamp_min(1e-8)
    # 计算整条seq条件概率，除以序列长度（公式中没有），是因为概率是累加的，防止长句子
    # 累加的概率绝对值变化越大，消除长度偏好
    ref_log_probs = (ref_log_probs * mask).sum(dim=1) / seq_lengths.squeeze()
    policy_log_probs = (policy_log_probs * mask).sum(dim=1) / seq_lengths.squeeze()
    # 张量解包，送入显卡中的batch是两倍的，一半是chosen，一半是reject
...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # batch size是两倍的，一半是chosen，一半是reject
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
# 开启自动混合精度（AMP）。将模型计算中的大矩阵乘法自动转换为半精度（如 FP16 或 BF16），
# 而保留梯度累加等关键步骤为 FP32
        with autocast_ctx:
            with torch.no_grad():
                # 冻结基座模型（Reference Model）
...
# 乘上大系数（Scale），防止混合精度loss下溢，在优化器更新前再缩小回来
